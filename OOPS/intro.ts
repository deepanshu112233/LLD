let greetings:string = "hi, Deep";

greetings=greetings.toUpperCase();
console.log(greetings);


let userId: number = 34645;
...
const doubled:number[]=nums.map((e)=>e*2)
console.log("doubled values:",doubled)

const sum:number=nums.reduce((acc,init)=>acc+init,0)
console.log("Nums array sum:",sum)

const mul:number=nums.reduce((acc,init)=>acc*init,1)
